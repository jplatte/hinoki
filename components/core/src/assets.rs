...
use self::file_config::{FileConfig, ProcessContent};
use crate::{
...
impl<'c: 'sc, 's, 'sc> AssetsProcessor<'c, 's, 'sc> {
    pub(crate) fn new(
        ...
        Self { assets_directory, metadata_env, render_scope, ctx }
    }

    pub(crate) fn run(&self) -> anyhow::Result<()> {
        self.process_assets_dir(&self.assets_directory, WriteOutput::Yes)?;
        Ok(())
    }

    pub(crate) fn dump(&self) -> anyhow::Result<()> {
        let metadata = self.process_assets_dir(&self.assets_directory, WriteOutput::No)?;
        println!("{metadata:#?}");
...
impl<'a> AssetsProcessorContext<'a> {
    pub(crate) fn new(output_dir_mgr: &'a OutputDirManager) -> Self {
        Self { output_dir_mgr, did_error: AtomicBool::new(false) }
    }
